...
const eventSchema = new mongoose.Schema({
    ...
    eventStartDate: { type: Number, required: [true, "請輸入活動開始時間"] },
    eventEndDate: { type: Number, required: [true, "請輸入活動結束時間"] },
    releaseDate: { type: Number, required: [true, "請輸入賣票時間"] },
    ...
    //     // enum: ['演唱會', '韓國團體'], 
    //     required: [true, "請輸入活動類別"] }],

    createdAt: { type: Date, default: Date.now, select: false },
    updateAt: { type: Date, default: Date.now, select: true }
});
